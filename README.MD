...
This package provides two implementations of Nango authentication clients:

1. A Python implementation (`NangoAuthClient.py`) that follows the `BaseAuthClient` interface
2. A TypeScript/Node.js implementation (`NangoClient.ts`) that provides a full-featured client for the Nango API

## Python Implementation

The Python implementation can be used as a drop-in replacement for other auth clients in the system.

### Installation

```bash
pip install nango-auth-client
```

### Usage

```python
from auth.clients.NangoAuthClient import NangoAuthClient

# Initialize the client
nango_client = NangoAuthClient(secret_key="your_nango_secret_key")

# Get user credentials
credentials = nango_client.get_user_credentials("github", "user123")

# Get OAuth configuration
oauth_config = nango_client.get_oauth_config("github")

# Save user credentials
nango_client.save_user_credentials("github", "user123", credentials)
```

## TypeScript/Node.js Implementation

The TypeScript implementation provides a comprehensive client for the Nango API.

### Installation

```bash
npm install nango-auth-client
```

### Usage

```typescript
import { Nango } from 'nango-auth-client';

// Initialize the client
const nango = new Nango({ secretKey: 'your_nango_secret_key' });

// Get a connection
const connection = await nango.getConnection('github', 'user123');

// List records from a sync
...
- `NANGO_HOST`: Nango API host URL (defaults to https://api.nango.dev)
