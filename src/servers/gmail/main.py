...
def create_server(user_id, api_key=None):
    ...
    async def handle_list_tools() -> list[Tool]:
        ...
        return [
            ...
            Tool(
                ...
                description="List emails as structured JSON for programmatic consumption. Returns machine-readable email objects with metadata, body, and attachment info. Ideal for triggers and automation workflows.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": "Gmail search query (e.g., 'is:unread', 'from:someone@example.com', 'newer_than:1d'). Default: 'in:inbox'",
                        },
                        "label_ids": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Filter by Gmail label IDs (e.g., ['INBOX', 'UNREAD']). Applied in addition to query.",
                        },
                        "max_results": {
                            "type": "integer",
                        ...
                        "include_body": {
                            "type": "boolean",
                            "description": "Include email body text/html in results (default: true)",
                        },
                        "include_attachments_info": {
                            "type": "boolean",
                            "description": "Include attachment metadata in results (default: true)",
                        },
                        "include_headers": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Additional headers to include (e.g., ['Reply-To', 'Message-ID', 'In-Reply-To']). From, To, Cc, Bcc, Subject, Date are always included.",
                        },
                    },
                    "required": [],
                },
    ...
    async def handle_call_tool(
        ...
        if name == "read_emails":
        ...
        elif name == "list_emails_structured":
            query = (arguments or {}).get("query", "in:inbox")
            label_ids = (arguments or {}).get("label_ids", None)
            max_results = min(int((arguments or {}).get("max_results", 10)), 100)
            include_body = (arguments or {}).get("include_body", True)
            include_attachments_info_flag = (arguments or {}).get(
                "include_attachments_info", True
            )
            extra_headers = (arguments or {}).get("include_headers", [])
            ...
            messages = results.get("messages", [])
            if not messages:
                result_data = {"emails": [], "resultCount": 0, "query": query}
                return [TextContent(type="text", text=json.dumps(result_data))]

            # Standard headers always extracted
            standard_headers = [
                "From",
                "To",
                "Cc",
            ...
            for message in messages:
                ...
                # Extract all requested headers into a dict
                headers = {}
                for header in msg.get("payload", {}).get("headers", []):
                ...
                # Add any extra requested headers
                if extra_headers:
                ...
                if include_attachments_info_flag:
                    payload = msg.get("payload", {})
                    attachments = get_attachments_info(payload)
